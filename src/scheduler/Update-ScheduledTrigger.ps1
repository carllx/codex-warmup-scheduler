...
# Manages Windows Scheduled Task triggers for Codex Warmup V2
# Sets single dynamic one-shot TimeTrigger + logon recovery trigger
# Retains WakeToRun = true, StartWhenAvailable = true, MultipleInstances = IgnoreNew
...
# If executed directly as script, run the function
if ($MyInvocation.InvocationName -ne '.' -and $MyInvocation.Line -notmatch '^\s*\.\s+') {
    Update-ScheduledTrigger -TargetDateTime $TargetDateTime -TaskName $TaskName -DryRun:$DryRun.IsPresent -ShadowMode:$ShadowMode.IsPresent
}
